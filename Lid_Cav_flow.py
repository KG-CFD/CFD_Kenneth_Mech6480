...
data1 = np.loadtxt('Lid_Cav_Yvelocities_2500.txt', delimiter=' ', usecols=(0, 3))
data2 = np.loadtxt('Lid_Cav_Xvelocities_2500.txt', delimiter=' ', usecols=(0, 3))
...
for steps in range(NUM_STEPS):
    ...
    if ((steps + 1) % PLOT_EVERY == 0):
        ...
        # Ensure uu, vv match the grid size (NX, NY)
        xx, yy = np.meshgrid(xnodes, ynodes, indexing='ij')  # Adjust grid to (NX, NY)
